// ZipFile.cs
// https://raw.githubusercontent.com/sankyprabhu/UnityZip/master/Assets/Script/ZipFile.cs
...
public class ZipFile 
{
    ...
    public static void UnZip(string filePath, byte[] data)
    {
        using (ZipInputStream s = new ZipInputStream(new MemoryStream(data)))
        {
            ...
            {
                #if UNITY_EDITOR
                // PMC // Debug.LogFormat("Entry Name: {0}", theEntry.Name);
                #endif
                ...
                // PMC
                #if UNITY_EDITOR
                Debug.LogFormat("Entry Name: {0} {1}", directoryName, fileName);
                #endif
                // End PMC
