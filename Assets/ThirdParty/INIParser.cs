// INIParser.cs
// from: https://assetstore.unity.com/packages/tools/advanced-ini-parser-23706
/*******************************
Version: 1.0
Project Boon
*******************************/
...
public class INIParser
{
    ...
    public void Open(string path)
    {
        ...
        {
            // PMC
            Debug.LogWarning("INI file " + path + " not found, creating new file in this location.");
            // end PMC
    ...
    private bool ParseKeyValuePair(string Line, ref string Key, ref string Value)
    {
        ...
        {
            // Line is a comment because it starts with # or ;
            return false;
        }
        // End PMC
    ...
    {
        // PMC
        // TODO: make this more resilliant, it barfed when a 404 page was passed in // P3
        // End PMC
    ...
    // PMC
    public int ReadValue(string SectionName, int Key, int DefaultValue)
    {
        return ReadValue(SectionName, Key.ToString(), DefaultValue);
    }

    public string ReadValue(string SectionName, int Key, string DefaultValue)
    {
        return ReadValue(SectionName, Key.ToString(), DefaultValue);
    }

    // Return a list of all the values from a single section, ignoring the key names
    public List<string> GetListOfSectionValues(string SectionName)
    {
        if (m_Sections != null && m_Sections.ContainsKey(SectionName))
        {
            return new List<string>(m_Sections[SectionName].Values);
        }
        return null;
    }

    // Return a dictionary of all key value pairs in a specific section (private until we need it)
    private Dictionary<string, string> GetSectionDictionary(string SectionName)
    {
        if (m_Sections != null && m_Sections.ContainsKey(SectionName))
        {
    ...
    // Return the raw dictionary of dictionaries (private until we need it)
    private Dictionary<string, Dictionary<string, string>> getDictionary()
    {
        return m_Sections;
    }
    // End PMC
